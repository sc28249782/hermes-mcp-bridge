...
- response ไม่มี filesystem path, credential, prompt, output หรือ audit content. หลังทดสอบ restore config จาก backup แล้ว `cmp` ผ่านแบบ byte-for-byte; Dev tunnel ถูกเริ่มกลับด้วย config ปกติ.
