# คู่มือเชิงเทคนิค: สถาปัตยกรรมและกลไก Hermes MCP Bridge v0.4.0

เอกสารนี้อธิบายฐาน Hermes ของ `hermes-mcp-bridge-v0.4.0.zip`; ส่วน Codex/WSL2 และ approval gate ดู `CODEX-WSL2-TH.md`
...
`tests/test_mcp.py` เปิด fake MCP server ผ่าน stdio แล้วตรวจ initialize, discovery 16 tools, annotations, model discovery, model-aware submit, usage tools, health, status, result และ foreign run rejection ส่วน `tests/test_codex.py` ตรวจ allowlist, symlink escape, sandbox mode และ write approval gate
